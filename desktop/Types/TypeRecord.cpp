...
void TypeRecord::GarbageCollect(SignatureCall &call) {
	auto idx = state.GetStructIndex(struct_name);
	...
	for (auto &f : str->fields) {
		if (f.type == Primitive::STRING) {
			...
			call.llvm.ClearPermString(call.ir->CreateLoad(call.llvm.TypeString, gep), call.ir);
		}
		i++;
	}
}

llvm::Type *TypeRecord::GetLLVMType(bool is_ref, SignatureCall &call) {
