...
void TypePrimitive::GarbageCollect(SignatureCall &call) {
	if (primitive_type == Primitive::STRING) {
		call.llvm.ClearPermString(
			call.ir->CreateLoad(call.llvm.TypeConversion(call.llvm.locals_type[name]),
								call.llvm.locals[name]), call.ir);
	}
}

llvm::Type *TypePrimitive::GetLLVMType(bool is_ref, SignatureCall &call) {
	if (!is_ref) {
