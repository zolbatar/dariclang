...
private:
    ...
    static void VariableError(ParserToken &t, std::string name) {
        throw CustomException(ExceptionType::COMPILER, t.line, t.char_position, "Variable error with '" + name + "'");
    }
    static void RecordNotFound(ParserToken &t, std::string name) {
        throw CustomException(ExceptionType::COMPILER, t.line, t.char_position, "Record '" + name + "' not found");
    }
