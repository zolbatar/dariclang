...
llvm::Value *Reference::GlobalIndexPtr(bool option_base,
    ...
    for (size_t i = 0; i < indices_val.size() - 1; i++) {
        ...
        // Get array entry from dimensions
        auto size_v = llvm::ArrayType::get(llvm.TypeInt, indices_val.size());
        auto ptr = ir->CreateGEP(size_v, glob, {llvm::ConstantInt::get(llvm.TypeInt, 0), llvm::ConstantInt::get(llvm.TypeInt, i)});
        auto size = ir->CreateLoad(llvm.TypeInt, ptr);
