...
class Reference
{
 public:
	...
	std::string GetName()
	{ return name; }
	[[nodiscard]] size_t GetRef() const
	{ return index; }
	InstanceType GetInstanceType()
	{ return instance_type; }
	void SetDataType(Primitive _data_type)
	{ data_type = _data_type; }
	Primitive GetDataType()
	{ return data_type; }
	void SetStructName(std::string _name)
	{ struct_name = std::move(_name); }
	std::string GetStructName()
	{ return struct_name; }
	void SetLLVMStructType(llvm::StructType* type)
	{ this->llvm_struct_type = type; }
	...
	void Convert(ValueType& vt, CompilerLLVM& llvm, llvm::IRBuilder<>* ir)
	{
		switch (GetInstanceType())
		{
		case InstanceType::PRIMITIVE:
			llvm.AutoConversion(ir, vt, GetDataType());
			break;
		default:
			assert(0);
			break;
	...
	{ return fields; }
	void SetFields(std::string _fields)
