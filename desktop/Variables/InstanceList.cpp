...
std::shared_ptr<Instance> InstanceList::Build(const std::string &name,
                                              ...
                                              bool is_ref) {
    auto size = llvm::ConstantInt::get(llvm.TypeInt, llvm.dl->getTypeAllocSize(llvm.TypeConversion(data_type)));
    auto create = llvm.CreateCall("list_init", ir, func, {size}, false, default_return_type);
    switch (scope) {
        case Scope::LOCAL: {
...
bool InstanceList::Set(llvm::Value *v, llvm::Value *idx, size_t field_index, CompilerLLVM &llvm, llvm::IRBuilder<> *ir) {
    switch (scope) {
        case Scope::GLOBAL:
