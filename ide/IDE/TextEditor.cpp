...
static std::list<std::string> library = {"Sleep",
                                         ...
                                         "Mode", "BankedOn", "BankedOff", "Origin", "ScreenWidth", "ScreenHeight",
                                         "Flip",
                                         "Cls", "Plot", "SetLineWidth", "Rectangle", "RectangleFilled", "Triangle",
                                         "TriangleFilled", "TriangleShaded",
                                         "Circle", "CircleFilled", "ClipOn", "ClipOff", "Line", "SetFG", "SetAlpha",
                                         "SetBG", "Colour", "CursorOn", "CursorOff",

                                         "LoadSprite", "DeleteSprite", "GrabSprite", "DrawSprite",

                                         "Object", "Shape", "DeleteShape", "SetCamera", "Translate", "Rotate", "Scale",
                                         "DeleteObject",
                                         "DeleteAllObjects", "ShadowsOn", "ShadowsOff", "Render", "Vertex",
                                         "VertexColour",
                                         "FaceColour", "Face", "LoadMesh", "SetAmbient", "SetDirectional", "SetRotate",
                                         "GetRotateX", "GetRotateY", "GetRotateZ",

                                         "LoadFont", "SetFont", "Text", "TextRight", "TextCentre",

                                         "Rnd", "RndF",

                                         "OpenIn", "OpenOut", "OpenUp", "Eof", "Close", "BGet", "BPut",
                                         "SGet", "Tell", "Seek", "Dir", "DirEntry", "Delete",

                                         "ClearEvents", "WaitKey", "WaitKeyChar", "KeyDown", "Get", "GetChar", "MouseX",
                                         "MouseY", "MouseState",

                                         "LoadSoundFont", "LoadSample", "PlaySample", "LoadMusic", "PlayMusic",
                                         "StopMusic", "SetVolume", "SetMusicVolume"
};
...
TextEditor::TextEditor()
        : mLineSpacing(1.0f), mUndoIndex(0), mTabSize(4), mOverwrite(false), mReadOnly(false), mWithinRender(false),
          ...
          mTextChanged(false), mColorizerEnabled(true), mTextStart(20.0f), mLeftMargin(10),
...
void TextEditor::Render() {
    ...
    if (!mLines.empty()) {
        float
                spaceSize = ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f, " ", nullptr,
                                                            nullptr).x;
        ...
        while (lineNo <= lineMax) {
            ...
            longest =
                    std::max(mTextStart + TextDistanceToLineStart(Coordinates(lineNo, GetLineMaxColumn(lineNo))),
                             longest);
            auto columnNo = 0;
            ...
            if (mState.mSelectionStart <= lineEndCoord)
                sstart =
                        mState.mSelectionStart > lineStartCoord ? TextDistanceToLineStart(mState.mSelectionStart)
                                                                : 0.0f;
            ...
            if (!line.empty()) {
                ...
                if (withinString) {
                    ...
                    if (c == '\"') {
                        ...
                        for (auto singleStartStr: mLanguageDefinition.mSingleLineComment) {
...
float TextEditor::TextDistanceToLineStart(const Coordinates &aFrom) const {
    ...
    for (size_t it = 0u; it < line.size() && it < static_cast<size_t>(colIndex);) {
        if (line[it].mChar == '\t') {
            distance =
                    (1.0f + std::floor((1.0f + distance) / (float(mTabSize) * spaceSize))) *
                    (float(mTabSize) * spaceSize);
            ++it;
        } else {
            ...
            tempCString[i] = '\0';
            distance +=
                    ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f, tempCString, nullptr,
                                                    nullptr).x;
        }
    }
...
const TextEditor::LanguageDefinition &TextEditor::LanguageDefinition::DARIC() {
    static bool inited = false;
    static LanguageDefinition langDef;
    if (!inited) {
//        langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("Def [a-zA-Z_][a-zA-Z0-9_]*", PaletteIndex::Def));
//        langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("(?:Def[ \t\n]*)([a-zA-Z_][a-zA-Z0-9_]*)", PaletteIndex::Def));
        static const char *const keywords[] = {
                "CASE", "CONST", "DATA", "DATALABEL", "DEF", "DIM",
                "ELSE", "END", "FOR", "IF", "IMPORT", "LET", "NEXT", "OPTION",
                "OTHERWISE", "PRINT", "READ", "RECORD", "BYREF",
                "REM", "REPEAT", "RESTORE", "RETURN", "THEN", "SIZE",
                "STEP", "SWAP", "TO", "UNTIL", "WHEN", "WITH", "WHILE",
                "SYS",

                "MOD", "DIV", "NOT", "AND", "OR", "EOR",

                "BYTE", "INT", "FLOAT", "STRING",

                "ABS", "ACS", "ASN", "ATN", "COS", "DEG", "EXP", "FLOOR",
                "LN", "LOG", "PI", "RAD", "ROUND", "SGN", "SIN", "SQR", "TAN",

                "TRUE", "FALSE",

                "ASC", "CHR", "INSTR", "LEFT", "MID", "RIGHT", "LEN"
        };
        for (auto &k: keywords)
        ...
        for (auto &k: keywords) {
            ...
            kk[0] = toupper(kk[0]);
            langDef.mKeywords.insert(kk);
        }

        auto identifiers = library;
        ...
        langDef.mTokenRegexStrings.push_back(
                std::make_pair<std::string, PaletteIndex>("&[0-9a-fA-F]+[uU]?[lL]?[lL]?", PaletteIndex::Number));
        langDef.mTokenRegexStrings.push_back(
...
//        langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("Def [a-zA-Z_][a-zA-Z0-9_]*", PaletteIndex::Def));
        ...
        langDef.mCommentEnd = "}#";
        langDef.mSingleLineComment.push_back("'");
        langDef.mSingleLineComment.push_back("REM");
        langDef.mSingleLineComment.push_back("Rem");
