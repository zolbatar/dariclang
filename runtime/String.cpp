...
extern "C" DLLEXTERN T_S clone_string(T_S v1) {
	auto m = (char *)malloc(strlen(v1) + 1);
	strcpy(m, v1);
#ifdef SDEBUG
	std::cout << "Cloning String: " << (size_t)v1 << " is " << v1 << "' to " << (size_t)m << std::endl;
#endif
	return m;
}
...
extern "C" DLLEXTERN void Add_Temp_String(T_S v) {
#ifdef SDEBUG
...
#ifdef SDEBUG
		std::cout << "FAILED Clear Perm String: " << (size_t)v << " is '" << v << "'" << std::endl;
#endif
	}
}
...
#ifdef SDEBUG
		std::cout << "FAILED Make Perm String: " << (size_t)v << " is '" << v << "'" << std::endl;
#endif
	}
}

extern "C" DLLEXTERN void Add_Perm_String(T_S v) {
#ifdef SDEBUG
	std::cout << "Add Perm String: " << (size_t)v << " is '" << v << "'" << std::endl;
#endif
	perm_strings.insert(v);
}
